...
ir::BasicBlock* PickNextBlock(std::vector<ir::BasicBlock*>& readyList, ir::BasicBlock* lastScheduled) {
    if (readyList.empty()) {
        return nullptr;
    }
...
void LinearOrderAnalyzer::CreateLinearOrder(ir::BasicBlock *startBlock) {
    ...
    while (!readyList.empty()) {
        ...
        if (readyList.empty() && (graph_.linearOrderedBlocks_.size() < blocks.size())) {
            while (fallbackIt != blocks.end() && visited.find(*fallbackIt) != visited.end()) {
                ++fallbackIt;
            }
