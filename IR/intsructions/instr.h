...
class Instr {
public:
    explicit Instr(InstrOpcode op) : op_(op), prev_(nullptr), next_(nullptr), inputs_(), users_() {}
    explicit Instr(InstrOpcode op, const std::vector<Instr *> &&inputs, const std::vector<Instr *> &&users) :
    ...
    void AddInput(Instr *input, size_t index) { inputs_.insert(inputs_.begin() + index, input); }
    void AddUser(Instr *user, size_t index) { users_.insert(users_.begin() + index, user); }
    ...
    void RemoveInput(Instr *input, size_t index) { inputs_.erase(inputs_.begin() + index); }
    void RemoveUser(Instr *user, size_t index) { users_.erase(users_.begin() + index); }
...
#define DECLARE_IS_CHECKS(instrType, className)                                                         \
    ...
    {                                                                                                   \
        return op_ == InstrOpcode::instrType;                                                           \
    }
