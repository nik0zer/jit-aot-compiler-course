...
void ShortenRangeStart(ir::instr::Instr *instr,
                       ir::instr::Instr::live_t blockFrom) {
...
void LivenessAnalyzer::CalculateLiveRanges() {
  ...
  for (auto it = blocks.rbegin(); it != blocks.rend(); ++it) {
    ...
    for (auto *succ : block->GetSuccs()) {
      ...
      if (predIndex == static_cast<size_t>(-1))
        continue;

      for (auto *phi = succ->GetFirstInstr();
           phi != nullptr && phi->IsPhiInstr(); phi = phi->GetNextInstr()) {
    ...
    for (auto *opd : live) {
      opd->GetLiveRange().AddInterval(
          {block->GetLiveRange().first, block->GetLiveRange().second});
    }

    for (auto *currentInstr = block->GetLastInstr(); currentInstr != nullptr;
         currentInstr = currentInstr->GetPrevInstr()) {
      ...
      for (auto *input : currentInstr->GetInputs()) {
        ASSERT(input != nullptr)
        input->GetLiveRange().AddUse(currentInstr->GetLiveRange().GetLive());
        input->GetLiveRange().AddInterval(
            {blockFrom, currentInstr->GetLiveRange().GetLive()});
        live.insert(input);
      }
    }

    for (auto *phi = block->GetFirstInstr();
         phi != nullptr && phi->IsPhiInstr(); phi = phi->GetNextInstr()) {
      live.erase(phi);
    }
