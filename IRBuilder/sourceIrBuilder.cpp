...
template <typename T> struct UnderProcessedConnection {
...
using UnderProcessedPredessor =
    std::vector<UnderProcessedConnection<ir::BasicBlock>>;
using UnderProcessedSuccesor =
    std::vector<UnderProcessedConnection<ir::BasicBlock>>;
using UnderProcessedInput =
    std::vector<UnderProcessedConnection<ir::instr::Instr>>;
...
ir::BasicBlock *
CheckIfBasicBlock(std::string_view line, ir::MethodGraph &graph,
  ...
  if (std::regex_match(str, match, bbRegex)) {
    ...
    if (blocksMap.count(currentBBId)) {
      diagnosticEngine.ThrowError("Basic block with id " +
                                      std::to_string(currentBBId) +
                                      " already exists",
                                  lineNum, file);
      return nullptr;
    }
    ...
    if (match[1].matched) {
      ...
      for (; it != end; ++it) {
        ...
        if (predIt != blocksMap.end()) {
          ...
          underProcessedConnections.unresolvedPreds.push_back(
              {newBB, preds.size() - 1, predId, {file, lineNum}});
        }
      }
    ...
    if (match[3].matched) {
      ...
      for (; it != end; ++it) {
        ...
        if (succIt != blocksMap.end()) {
          ...
          underProcessedConnections.unresolvedSuccs.push_back(
              {newBB, i, succId, {file, lineNum}});
        }
        i++;
      }
...
ir::instr::TypeId StringToTypeId(const std::string &type) {
  if (type == "u8")
    return ir::instr::TypeId::U8;
  if (type == "u16")
    return ir::instr::TypeId::U16;
  if (type == "u32")
    return ir::instr::TypeId::U32;
  if (type == "u64")
    return ir::instr::TypeId::U64;
  if (type == "i8")
    return ir::instr::TypeId::I8;
  if (type == "i16")
    return ir::instr::TypeId::I16;
  if (type == "i32")
    return ir::instr::TypeId::I32;
  if (type == "i64")
    return ir::instr::TypeId::I64;
  if (type == "f32")
    return ir::instr::TypeId::F32;
  if (type == "f64")
...
ir::instr::BinaryOperationType StringToBinaryOpType(const std::string &op) {
  if (op == "add")
    return ir::instr::BinaryOperationType::ADD;
  if (op == "sub")
    return ir::instr::BinaryOperationType::SUB;
  if (op == "mul")
    return ir::instr::BinaryOperationType::MUL;
  if (op == "div")
    return ir::instr::BinaryOperationType::DIV;
  if (op == "and")
    return ir::instr::BinaryOperationType::AND;
  if (op == "or")
    return ir::instr::BinaryOperationType::OR;
  return ir::instr::BinaryOperationType::ADD; // Should not happen
}

ir::instr::IfType StringToIfType(const std::string &type) {
  if (type == "eq")
    return ir::instr::IfType::EQ;
  if (type == "ne")
    return ir::instr::IfType::NE;
  if (type == "lt")
    return ir::instr::IfType::LT;
  if (type == "le")
    return ir::instr::IfType::LE;
  if (type == "gt")
    return ir::instr::IfType::GT;
  if (type == "ge")
    return ir::instr::IfType::GE;
  return ir::instr::IfType::EQ; // Should not happen
}

using InstrParserFunc =
    ir::instr::Instr *(*)(const std::string &,
                          std::unordered_map<size_t, ir::instr::Instr *> &,
                          UnderProcessedConnections &, size_t,
                          std::filesystem::path &, DiagnosticsEngine &);

ir::instr::Instr *
AddParsedInstr(ir::instr::Instr *instr, size_t id,
...
ir::instr::Instr *
ParseParamInstr(const std::string &line,
                std::unordered_map<size_t, ir::instr::Instr *> &instrMap,
                UnderProcessedConnections &underProcessedConnections,
                size_t lineNum, std::filesystem::path &file,
                DiagnosticsEngine &diagnosticEngine) {
...
ir::instr::Instr *
ParseConstantInstr(const std::string &line,
                   std::unordered_map<size_t, ir::instr::Instr *> &instrMap,
                   UnderProcessedConnections &underProcessedConnections,
                   size_t lineNum, std::filesystem::path &file,
                   DiagnosticsEngine &diagnosticEngine) {
...
ir::instr::Instr *
ParseCastInstr(const std::string &line,
               std::unordered_map<size_t, ir::instr::Instr *> &instrMap,
               UnderProcessedConnections &underProcessedConnections,
               size_t lineNum, std::filesystem::path &file,
               DiagnosticsEngine &diagnosticEngine) {
...
ir::instr::Instr *ParseBinaryOperationInstr(
  ...
  std::smatch match;
  std::regex binOpRegex(
      R"((\d+)\.(u\d+|void)\s+(add|sub|mul|div|and|or) v(\d+) v(\d+))");
  if (std::regex_match(line, match, binOpRegex)) {
    ...
    auto newInstr =
        new ir::instr::BinaryOperationInstr(type, op, input1, input2);

    if (input1 == nullptr) {
      underProcessedConnections.unresolvedInputs.push_back(
          {newInstr, 0, input1Id, {file, lineNum}});
    }
    if (input2 == nullptr) {
      underProcessedConnections.unresolvedInputs.push_back(
          {newInstr, 1, input2Id, {file, lineNum}});
    }
...
ir::instr::Instr *
ParseReturnInstr(const std::string &line,
                 std::unordered_map<size_t, ir::instr::Instr *> &instrMap,
                 UnderProcessedConnections &underProcessedConnections,
                 size_t lineNum, std::filesystem::path &file,
                 DiagnosticsEngine &diagnosticEngine) {
  ...
  if (std::regex_match(line, match, retRegex)) {
    ...
    if (input == nullptr && match[3].matched) {
      underProcessedConnections.unresolvedInputs.push_back(
          {newInstr, 0, std::stoul(match[3]), {file, lineNum}});
    }
    return AddParsedInstr(newInstr, instrId, instrMap, lineNum, file,
                          diagnosticEngine);
  }
...
ir::instr::Instr *
ParsePhiInstr(const std::string &line,
              std::unordered_map<size_t, ir::instr::Instr *> &instrMap,
              UnderProcessedConnections &underProcessedConnections,
              size_t lineNum, std::filesystem::path &file,
              DiagnosticsEngine &diagnosticEngine) {
...
ir::instr::Instr *
ParseIfInstr(const std::string &line,
             std::unordered_map<size_t, ir::instr::Instr *> &instrMap,
             UnderProcessedConnections &underProcessedConnections,
             size_t lineNum, std::filesystem::path &file,
             DiagnosticsEngine &diagnosticEngine) {
  std::smatch match;
  std::regex ifRegex(
      R"((\d+)\.(u\d+|void)\s+if\.(gt|lt|ge|le|eq|ne) v(\d+) v(\d+))");
...
CheckIfInstruction(std::string_view line,
  ...
  for (const auto &parser : InstrParsers) {
    if (ir::instr::Instr *instr =
            parser(str, instrMap, underProcessedConnections, lineNum, file,
                   diagnosticEngine)) {
      return instr;
    }
  }
...
public:
  ...
  void AddFlag(ProcessedInstrTypes type) {
    processedInstrTypes_ = static_cast<ProcessedInstrTypes>(
        static_cast<uint64_t>(processedInstrTypes_) |
        static_cast<uint64_t>(type));
  }
  bool HasFlag(ProcessedInstrTypes type) const {
    return (static_cast<uint64_t>(processedInstrTypes_) &
            static_cast<uint64_t>(type)) != 0U;
  }
...
static bool CheckInstrOrderErrors(ir::BasicBlock *currentBlock,
                                  BasicBlockParseState &bbParseState,
                                  ir::instr::Instr *instr,
                                  DiagnosticsEngine::Position &&position,
                                  DiagnosticsEngine &diagnosticEngine) {
  if (currentBlock == nullptr) {
    diagnosticEngine.ThrowError("Instruction outside of a basic block",
                                position);
    return false;
  }
  if (bbParseState.HasFlag(ProcessedInstrTypes::NonPhi) &&
      instr->GetOpcode() == ir::instr::InstrOpcode::PHI) {
...
ir::MethodGraph *SourceIrBuilder::Build(std::ostream &diagnosticOutput) {
  ...
  while (std::getline(*source, line)) {
    ...
    if (auto block = CheckIfBasicBlock(line, *graph, blocksMap,
                                       underProcessedConnections, currentLine,
                                       sourcePath_, diagnosticEngine);
        block != nullptr) {
      currentBlock = block;
    ...
    if (auto instr =
            CheckIfInstruction(line, instrMap, underProcessedConnections,
                               currentLine, sourcePath_, diagnosticEngine);
        instr != nullptr) {
      if (!CheckInstrOrderErrors(currentBlock, bbParseState, instr,
                                 {sourcePath_, currentLine},
                                 diagnosticEngine)) {
        continue;
      }
  ...
  auto checkIfUnresolved = [&](auto &it, auto &map, auto &connection,
                               ...
                               size_t objectId) -> bool {
    if (it == map.end()) {
      std::stringstream ss;
      ss << "Unresolved object " << objectType << " : "
         << std::to_string(objectId);
      diagnosticEngine.ThrowError(ss.str(), connection.position.line,
                                  connection.position.file);
      return false;
    }
    return true;
  };
  for (auto &predConnection : underProcessedConnections.unresolvedPreds) {
    ...
    if (!checkIfUnresolved(bbIt, blocksMap, predConnection, "bb pred",
                           predConnection.inputObjectId)) {
      continue;
    }
    predConnection.objectWithUnresolvedConnection->SetPred(
        bbIt->second, predConnection.numOfInput);
  }
  for (auto &succConnection : underProcessedConnections.unresolvedSuccs) {
    auto bbIt = blocksMap.find(succConnection.inputObjectId);
    if (!checkIfUnresolved(bbIt, blocksMap, succConnection, "bb succ",
                           succConnection.inputObjectId)) {
      continue;
    }
    succConnection.objectWithUnresolvedConnection->SetSucc(
        bbIt->second, succConnection.numOfInput);
  }
  for (auto &input : underProcessedConnections.unresolvedInputs) {
    auto instrIt = instrMap.find(input.inputObjectId);
    if (!checkIfUnresolved(instrIt, instrMap, input, "instr",
                           input.inputObjectId)) {
      continue;
    }
